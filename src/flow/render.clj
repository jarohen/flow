...
(defn render-el [{:keys [deps el-bindings el-init el-return on-update]} {:keys [updated-var-sym old-state-sym new-state-sym]}]
  `(do
       ...
       ~(when (seq deps)
          (let [!state (gensym "!state")
            ...
            `(let [~!state (atom {})
               ...
               (letfn [(notify!# [~updated-var-sym ~old-state-sym ~new-state-sym]
                         ~@on-update)
