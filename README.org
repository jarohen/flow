...
- *It's as close to ClojureScript as possible, to minimise the
  ...
  =if=, =when=, =when-let=, etc all work as they do in ClojureScript,
  albeit having been extended to handle continuous dynamic updates, so
  there shouldn't be any surprises for anyone who knows CLJS.
...
/* - Flow's 'for' doesn't currently support ':when', ':let', or
':while' clauses - support for these is planned for a future release./
...
- [[https://github.com/james-henderson/flow/tree/counter/samples/counter][Flow in 5 Minutes]] - writing a 'Hello World'/simple counter
  application
...
- [[https://github.com/james-henderson/flow/tree/0.2.0-a6/samples/todomvc][TodoMVC]] example application - no webapp library would be complete
  without it!
